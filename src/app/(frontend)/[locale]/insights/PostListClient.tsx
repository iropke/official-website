...
function Pagination({ pagination, locale, basePath }: PaginationProps) {
...
/* ── Client component ── */
/* TODO: Replace with Payload CMS data fetching via `payload.find({ collection: 'posts' })` */
interface PostListClientProps {
  locale: string;
  currentPage: number;
}

export default function PostListClient({ locale, currentPage }: PostListClientProps) {
  const sectionRef = useRef<HTMLElement>(null);

  useRevealObserver(sectionRef);

  /* Placeholder posts — will be replaced by Payload CMS query */
  const posts: Post[] = Array.from({ length: 20 }, (_, i) => ({
    slug: `post-${i + 1}`,
    title: `Sample post title ${i + 1}`,
    description:
      'This is a placeholder description for the post card. It will be replaced with real content from Payload CMS.',
    date: 'Apr 06, 2026',
    thumbnailUrl: 'https://placehold.co/540x398',
    thumbnailAlt: `Abstract editorial thumbnail for post ${i + 1}`,
  }));
