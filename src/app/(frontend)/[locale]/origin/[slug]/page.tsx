...
const CATEGORY = 'origin' as const
const CATEGORY_PATH = '/origin'
...
export default async function OriginDetailPage({ params, searchParams }: PageProps) {
  const { locale: rawLocale, slug } = await params
  const locale = normalizeLocale(rawLocale)
  ...
  if (search.preview === 'true') {
    try {
      ...
      console.error('[OriginDetailPage] Preview auth check failed:', err)
    }
  }
  ...
  try {
    ...
    console.error('[OriginDetailPage] Failed to load post:', err)
  }
  ...
  try {
    ...
    console.error('[OriginDetailPage] Failed to load related posts:', err)
  }
