...
export default async function StoryDetailPage({ params, searchParams }: PageProps) {
  ...
  if (search.preview === 'true') {
    try {
      ...
      console.error('[StoryDetailPage] Preview auth check failed:', err)
    }
  }
  ...
  try {
    ...
    console.error('[StoryDetailPage] Failed to load post:', err)
  }
  ...
  try {
    ...
    console.error('[StoryDetailPage] Failed to load related posts:', err)
  }
