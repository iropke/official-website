...
/**
 * 언어별 sub-sitemap 분리 (locale 별 1 sitemap, 총 20개).
 *
 * 정책 결정 (2026-05-09): 단일 sitemap.xml 에 20 개 locale 을 섞어 두면
 ...
 *   /sitemap/ja.xml ...   (총 20 개)
 *
 * sitemap-index 정책 (2026-05-12): Next.js 16 의 metadata file API 는
 * `generateSitemaps` 사용 시 sitemap-index (`/sitemap.xml`) 를 자동 생성하지
 * 않는다 (vercel/next.js#77304). 별도 Route Handler 로 만드는 대신,
 * `robots.txt` 에 20개 sub-sitemap URL 을 모두 명시하고 GSC/Bing 에도
 * 20개 URL 을 직접 제출한다 (D-7 체크리스트). 색인 결과는 sitemap-index
 * 사용 시와 동일.
 *
