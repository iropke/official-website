  ...
  ko: [
    ...
    },
    {
      ...
      note:
        ...
        '— nonsensical). "consistent bylines across articles" → "모든 아티클에서 일관된 작성자 표기".',
    },
    {
      source: 'companion spoke / the companion spoke on',
      target: '관련 아티클 / 함께 보면 좋은 아티클',
      note:
        ...
        '"[X] 를 다루는 관련 아티클에서는 … 다룹니다". NEVER 동료가 말했습니다 / 동료는 다음과 같이 말했습니다 ' +
        '(reading "spoke" as a verb and "companion" as a colleague — a full mistranslation). ' +
        'NEVER 기사 (newspaper / news article register, wrong for editorial — see article entry).',
    },
    ...
    // ── v3.3 (2026-05-26) — structure parallel notation + article 표준 음역 ──
    {
      source: 'structure / structures (AEO/SEO/IA core term context)',
      target: '구조(structure) — 매 등장마다 영문 병기 (passage / surface / indexing / citation 동일 패턴)',
      note:
        'CRITICAL — SAME PATTERN AS passage / surface / indexing / crawl-access / citation. ' +
        'When "structure" appears as an AEO/SEO/IA core term (heading structure, content structure, ' +
        'site structure, page structure, information structure, passage structure, URL structure, ' +
        'data structure in JSON-LD/schema contexts, navigation structure), render as 구조(structure): ' +
        'the Korean term 구조 followed immediately by the English word "structure" in parentheses, ' +
        'NO space before "(". Korean particles attach after the closing paren: "heading structure" → ' +
        '"제목 구조(structure)", "the page structure determines extraction" → ' +
        '"페이지 구조(structure)는 추출 가능성을 결정합니다". Required on EVERY occurrence. ' +
        'EXCEPTION 1 — VERB FORMS keep bare 구조: "structured content" → "구조화된 콘텐츠" (NOT ' +
        '구조(structure)화된 — verb conjugation with English anchor is awkward), "to structure" → ' +
        '"구조화하다" (NOT 구조(structure)화하다). Same rule as citation noun-only. ' +
        'EXCEPTION 2 — GENERAL-LANGUAGE COMPOUNDS use plain 구조 with NO English anchor: ' +
        '"organizational structure" → 조직 구조, "company structure" → 회사 구조, ' +
        '"pricing structure" → 가격 구조, "team structure" → 팀 구조, "cost structure" → 비용 구조, ' +
        '"legal structure" → 법적 구조. These are NOT AEO/SEO/IA core terms and the English anchor ' +
        'would be redundant. ' +
        'EXCEPTION 3 — close repetition: bare 구조 acceptable after one parenthetical anchor in the ' +
        'same sentence (e.g. "콘텐츠 구조(structure)를 잡으면, 그 구조가…"). ' +
        'When uncertain (AEO/SEO/IA context vs general), DEFAULT TO parallel notation — over-marking ' +
        'is cheaper to walk back than under-marking. NEVER bare 구조 in AEO/SEO/IA core term contexts ' +
        'without the English anchor — loses precise AEO term mapping (same loss as bare 표면 / 인용).',
    },
    {
      source: 'article / articles',
      target: '아티클',
      note:
        'CRITICAL — ALL Korean renderings of "article / articles" must be 아티클, NEVER 기사. ' +
        '기사 carries a newspaper / news-article register that is wrong for editorial tone. ' +
        '아티클 is the established industry transliteration used by Korean tech / editorial publishers. ' +
        'Applies to noun + compound + every context: "this article" → 이 아티클, ' +
        '"the article covers" → 이 아티클은 ~ 다룹니다, "related article" → 관련 아티클, ' +
        '"companion article / companion spoke" → 관련 아티클 / 함께 보면 좋은 아티클, ' +
        '"articles in this cluster" → 이 클러스터의 아티클, "the original article" → 원문 아티클, ' +
        '"across articles" → 모든 아티클에서, "Insights articles" → Insights 아티클. ' +
        'NEVER 기사 / 글 (글 is too informal for editorial register; 기사 carries news connotation).',
    },
  ],
  ja: [
    {
...
export const EXAMPLES: Partial<Record<Locale, FewShotExample[]>> = {
  ko: [
    ...
    // article → 아티클 (v3.3), NOT 기사
    {
      en: 'This site uses a labeled TL;DR pattern across every Insights article.',
      target: '이롭게 공식 웹사이트는 모든 Insights 아티클에서 레이블이 붙은 TL;DR 패턴을 사용합니다.',
    },
    ...
    // ── v3.3 (2026-05-26) — structure parallel notation + article 표준 음역 ──
    // "heading structure" (AEO/SEO core term context) — 구조(structure) parallel notation
    {
      en: 'A clear heading structure helps AI engines extract passages from the page.',
      target:
        '명확한 제목 구조(structure)는 AI 엔진이 페이지에서 발췌 단위(passage)를 추출하는 데 도움이 됩니다.',
    },
    // "page structure" + close-repetition bare 구조 (after one anchor in same sentence)
    {
      en: 'The page structure determines extractability; that structure also shapes how citations appear.',
      target:
        '페이지 구조(structure)는 추출 가능성을 결정하며, 그 구조는 인용(citation)이 노출되는 방식에도 영향을 줍니다.',
    },
    // EXCEPTION 1 — verb form "structured" keeps bare 구조 (NOT 구조(structure)화된)
    {
      en: 'Structured content is easier for AI engines to cite.',
      target: '구조화된 콘텐츠는 AI 엔진이 인용(citation)하기에 더 쉽습니다.',
    },
    // EXCEPTION 2 — general-language compound uses plain 구조 (NO English anchor)
    {
      en: 'The organizational structure changed after the merger.',
      target: '합병 이후 조직 구조가 바뀌었습니다.',
    },
    // "article" → 아티클 (NOT 기사) — companion spoke / related article
    {
      en: 'The companion spoke on AEO measurement covers attribution in detail.',
      target: 'AEO 측정을 다루는 관련 아티클에서는 출처 추적을 자세히 설명합니다.',
    },
    // "across articles" → "모든 아티클에서" (NOT 기사)
    {
      en: 'Consistent bylines across articles strengthen the entity signal.',
      target: '모든 아티클에서 일관된 작성자 표기를 유지하면 엔티티 신호가 강화됩니다.',
    },
  ],
  ja: [
